...
resps:any;
	...
	message: "This trip will be cancelled soon if the rider refuses to confirm the end of the trip...",
	duration: 3000
	});
	toast.present();
	...
	expiry(){
		...
		this.bmpapiProvider.expire(bmp_jwt,rideId).subscribe(resp=>{
			...
			if(this.res.message==="expired"){
				let loading = this.loadingcontroller.create({
					content: "The rider did not respond after 3 minutes from the end of the trip, stopping the trip.."
				});
				loading.present();
	...
	fare() {
		...
		this.bmpapiProvider.end_trip(bmp_jwt,ride_id).subscribe(response => {
			loading.dismiss();
			this.response = response;
