...
import { Platform, NavController, ModalController, ToastController, AlertController, App,LoadingController} from 'ionic-angular';
...
timer:any;
  ...
  public foregroundService: ForegroundService,
   private ringtones: NativeRingtones,private powerManagement: PowerManagement, private loadingcontroller: LoadingController) {
...
load(){
  ...
  //  console.log(this.lookup_triedIDs);
    ...
    this.bmpapiProvider.triplookup(jwt,this.lookup_triedIDs).subscribe((response:any)=> {
      this.success = response;
      console.log('trips here: ',JSON.stringify(response.message.details));
      //console.log(this.success.data.ride_id);
	  //get the status of isExecutive
	  if(response.message.details.isExecutive=='1' && !this.isExecutive){
		//pass
		console.log('failed cos driver is not an executive driver')
	  }
	  else{
		console.log('now here',response.message.details);
      if (this.success.data.ride_id != null){
        if(this.lookup_triedIDs == ""){
          ...
          this.show_notify();
          this.app.getRootNav().setRoot(RequestPage,{ride_data: this.success.data.details,details:response.message.details});
...
show_timer(){
  ...
  this.localNotifications.schedule({
    id:2,
        text:"Écouter le trajet pendant que lapplication sexécute en arrière-plan",
    // silent:t,
    data:{test:"sample"}
  })
}
...
// this.nativeAudio.play('uniqueId1');
  ...
  let user_id = bmp_userdata.user_id;
  this.bmpapiProvider.get_user(user_id,JSON.parse(localStorage.getItem("bmp_jwt"))).subscribe((res:any)=>{
	loading.dismiss()
...
console.log('Driver status',this.isExecutive)
  })
...
this.powerManagement.acquire().then(()=>{
  ...
  this.lookup_triedIDsXX = 0;
    this.show_timer();
    this.lookup_triedIDsXX = this.lookup_triedIDsXX+1;
    console.log('here',this.lookup_triedIDsXX);
...
console.log('error here',err);
    ...
    if (this.checked){
      msg = "Êtes-vous sûr de vouloir vous connecter à Internet?";
    }else{
      msg = "Êtes-vous sûr de vouloir vous déconnecter ?";
    }

		let alert = this.alertCtrl.create({
			title: 'Confirmer le mode de visibilité',
			message: msg,
			buttons: [
				{
					text: 'No',
